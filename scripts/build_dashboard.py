# -*- coding: utf-8 -*-
"""估值雷达 · 终端仪表盘生成器 v11（UI 降噪与层级重构）

v11 视觉减法（2026-09-24/25 用户需求：界面乱、指标与 K 线估值标注不好看）：
  · K线左缘 S/R 徽章：价格 fmt0 取整 → fmtSR 自适应小数（<10 两位/<100 一位），
    删除徽章内方法缩写文本（明细在 hover/数据墙），徽章收窄约 40%；>5 条只标 A/B 级；pack 间距 22→24px
  · K线右缘：标签槽 GUT 84→96px；MA 端点改 M5/M10 短格式、字号 10.5；斐波那契标签默认不上图
    （数值在十字光标提示与数据墙明细）；packLblMutual 间距 17/18→19/20；Y 轴毛玻璃面板改实底
  · 估值色带：填充透明度降至 .06/.045/.05（hover .10）；删除图内"高估区/合理区/低估区"大字，
    改色带左缘 3px 竖色条 + 现价卡表达；现价卡加投影；越界 V_high/V_low 增加 >0 校验
    （指数页不再显示 ¥0 徽章）；修复 toggleBand 只重绘 Canvas 不同步 SVG 覆盖层的问题
  · Hero：价格与涨跌幅同行基线对齐；估值结论 chip 实心高亮、板块/PE分位降灰字 meta-txt；
    不可执行信息改 alert-warn 信息条；MOS 端标签简化
  · 数据墙：w-title 左 3px 色条；数据质量等级改实心徽章；sr-track 点位 >7 只标 A/B；折叠按钮旋转 -90°
  · 左导航：价格列定宽 58px 右对齐不再截断；代码列隐藏（title 提示完整代码）
  · 大盘总览：micro-row 指标区卡片化 stat-grid（3 列，≤640px 降 2 列）；
    修复指数"当日成交"单位 bug（成交量 → 亿手，标签改 当日量(手)）；缺失值改 --meta 灰；
    拥挤度刻度改线性绝对定位（与 cgPos 对齐）；历史极值 chips 单行横滚；板块表 gap 收紧、涨跌 pill 改色字
  · 底部 S/R 胶囊编号修正（按支撑/压力各自计数）；>6 个收起为"共 N 个"
  数据口径、S/R 生成逻辑、估值引擎均未改动；verify_decision_data.py 回归 PASS（52/52）。
  before/after 截图存档：snapshots_ui/before_ui2_*.png · after_ui2_*.png
...
.sb-search{margin:0 10px 8px;border:1px solid var(--hair);border-radius:6px;padding:7px 10px;font-size:13px;background:var(--bg);transition:border-color var(--dur-instant) var(--ease-instant)}
.sb-search:focus{outline:none;border-color:var(--blue);box-shadow:0 0 0 3px rgba(0,113,227,.12)}
...
.sb-grp{position:sticky;top:0;z-index:3;display:flex;align-items:center;justify-content:space-between;gap:8px;
  ...
  color:var(--meta);text-transform:uppercase;border-bottom:1px solid var(--hair)}
...
.sb-price-group{justify-content:flex-end;flex:none;width:58px}
.sb-zone-group{justify-content:flex-end}
/* 文本统一截断，防止长名撑破 */
...
.hero-strip .pills{display:flex;gap:8px;flex-wrap:wrap;margin-top:4px;align-items:center}
.meta-txt{font-size:12px;color:var(--meta);white-space:nowrap;line-height:1.5}
...
.hero-strip .pct{display:inline-block;margin-left:10px;font-size:17px;font-family:var(--font-mono);font-variant-numeric:tabular-nums;vertical-align:baseline}
...
.mos-txt.alert-warn{margin-top:8px;padding:5px 10px;background:rgba(215,0,21,.06);border-left:3px solid var(--red-d);border-radius:0 6px 6px 0;color:var(--red-d);font-weight:500}
...
.cong-peaks{display:flex;flex-wrap:nowrap;gap:5px 10px;font-size:12px;line-height:1.6;margin-top:7px;overflow-x:auto;padding-bottom:2px;scrollbar-width:thin}
...
/* ===== 总览指标 stat-grid（P5：micro-row 卡片化，统一 3 列节奏） =====*/
.ov-wrap .micro-row{border:1px solid var(--hair);border-radius:10px;background:var(--bg2);padding:10px 12px!important;display:grid;grid-template-columns:repeat(3,1fr);gap:10px 16px;margin:8px 0}
.ov-wrap .micro-row .micro .k{font-size:11px;color:var(--meta);letter-spacing:.02em}
.ov-wrap .micro-row .micro .v{font-size:15px!important;margin-top:2px}
.ov-wrap .micro-row .micro .m{font-size:11px;margin-top:1px}
@media(max-width:640px){.ov-wrap .micro-row{grid-template-columns:repeat(2,1fr)}}
/* 第三层：五类状态分组 */
...
/* 行业板块行 */
.sec-row{display:grid;grid-template-columns:minmax(72px,auto) auto minmax(0,1fr) minmax(88px,auto);gap:6px;align-items:center;
...
.w-title{font-size:var(--fs-title);font-weight:600;letter-spacing:.05em;text-transform:uppercase;
  color:var(--sub);margin-bottom:12px;display:flex;align-items:center;gap:6px;line-height:1.5;min-width:0;border-left:3px solid var(--blue);padding-left:8px}
...
.w-card.collapsed .w-fold{color:var(--meta);transform:rotate(-90deg)}
  ...
  <main class="center" id="center">
    <div class="hero-strip" id="heroStrip">
      ...
      <div class="hr">
        ...
        <div class="mos-wrap">
          <div class="mos-track"><span>低估</span><span>安全边际</span><span>高估</span></div>
          <div class="mos-bar">
...
/* S/R 徽章自适应小数：<10 两位、<100 一位、否则整数，避免 "¥7/¥7/¥7" 无法区分 */
const fmtSR = v => v==null ? '—' : (+v).toLocaleString('zh-CN',{maximumFractionDigits: (+v) < 10 ? 2 : ((+v) < 100 ? 1 : 0)});
...
function renderList(filter){
  ...
  el.innerHTML = groups.map(g => {
      ...
      const barTitle = (s.v_low!=null && s.v_high!=null && s.v_high>s.v_low)
        ...
        : '无估值区间';
      return '<button class="sb-item '+(s.ticker===CUR?.ticker&&VIEW==='stock'?'on':'')+'" title="'+s.name+' '+s.ticker+'" onclick="switchStock(\''+s.ticker+'\')">'
...
function renderOvMarket(){
  ...
  if(idx){
    ...
    if(d){
      ...
      tech = '<div class="micro-row" style="border-top:none;padding-top:0">'
        + '<div class="micro"><div class="k">上证 5日</div><div class="v" style="font-size:14px;color:' + (d.r5==null?'var(--meta)':(d.r5>=0?'var(--green-d)':'var(--red-d)')) + '">' + tf(d.r5) + '</div></div>'
        + '<div class="micro"><div class="k">上证 10日</div><div class="v" style="font-size:14px;color:' + (d.r10==null?'var(--meta)':(d.r10>=0?'var(--green-d)':'var(--red-d)')) + '">' + tf(d.r10) + '</div></div>'
        + '<div class="micro"><div class="k">上证 20日</div><div class="v" style="font-size:14px;color:' + (d.r20==null?'var(--meta)':(d.r20>=0?'var(--green-d)':'var(--red-d)')) + '">' + tf(d.r20) + '</div></div>'
        + '<div class="micro"><div class="k">上证 60日</div><div class="v" style="font-size:14px;color:' + (d.r60==null?'var(--meta)':(d.r60>=0?'var(--green-d)':'var(--red-d)')) + '">' + tf(d.r60) + '</div></div>'
  ...
  if(idx){
    ...
    if(d && closes.length){
      ...
      const amtTxt = last && last.v != null ? (last.v/1e8).toFixed(1)+'亿手' : '—';
      depth = '<div class="micro-row" style="border-top:1px solid var(--hair);padding-top:8px">'
        ...
        + '<div class="micro"><div class="k">当日涨跌</div><div class="v" style="font-size:15px;color:' + (d.chg==null?'var(--meta)':(d.chg>=0?'var(--green-d)':'var(--red-d)')) + '">' + (d.chg!=null?(d.chg>=0?'+':'')+d.chg.toFixed(2)+'%':'—') + '</div></div>'
        + '<div class="micro"><div class="k">当日量(手)</div><div class="v" style="font-size:15px">' + amtTxt + '</div></div>'
  ...
  if(cg.value != null){
    ...
    congestionHTML = '<div class="cong-box">'
      ...
      + '<div class="cong-labels" style="position:relative;height:14px"><span style="position:absolute;left:0">30%</span><span style="position:absolute;left:33.3%;transform:translateX(-50%)">40% 关注</span><span style="position:absolute;left:50%;transform:translateX(-50%)">45% 警戒</span><span style="position:absolute;right:0">60%</span></div>'
      + '<div class="b-note" style="color:' + cgStat.c + '">' + cgNote + '</div>'
      + (peaks.length ? '<div class="b-note" style="color:var(--sub2);margin-top:6px">历史极值（乐咕 ' + (cgHist.points||0) + ' 点 · ' + (cgHist.first_date||'—') + '~' + (cgHist.last_date||'—') + '）≥45% 前7轮：</div><div class="cong-peaks">'
...
function renderHero(st, prevPrice, prevPct){
  ...
  const pills = [];
  if(st.sector) pills.push('<span class="meta-txt">'+st.sector+'</span>');
  pills.push('<span class="pill" style="background:'+zcol+';border-color:transparent;color:#fff">'+zm.label+'</span>');
  ...
  if(st.pe_ttm!=null && st.pe_ttm>0){ const ph=(DATA.pe_history||{})[st.ticker];
    if(ph && ph.ok && ph.pctile!=null) pills.push('<span class="meta-txt">PE分位 '+Math.round(ph.pctile*100)+'%</span>'); }
  ...
  if(raw != null && (st.decision_usable || st.reference_zone)){
    ...
    { const _mt = document.getElementById('mosTxt'); _mt.textContent = concl; _mt.className = 'mos-txt' + (st.decision_usable || st.decision_status === 'observe' ? '' : ' alert-warn'); }
  } else {
...
function renderWall(st){
  ...
  /* 点位 >7 时轨道只标 A/B 级（与 K 线标注策略一致），C 级明细仍在下方列表 */
  if(dots.length > 7){ const ab = dots.filter(d => d.cls.charAt(1) !== 'C'); if(ab.length >= 4) dots.splice(0, dots.length, ...ab); }
  ...
  document.getElementById('c1Body').innerHTML =
    ...
    + '<div class="micro"><div class="k">数据质量</div><div class="v"><span style="display:inline-block;min-width:22px;text-align:center;border-radius:6px;padding:1px 7px;font-size:13px;font-weight:700;color:#fff;background:'+(grade==='A'?'var(--blue)':(grade==='B'?'var(--green-d)':(grade==='C'?'var(--gold)':'var(--red-d)')))+'">'+grade+'</span></div>'
...
class ValuationChartEngine {
  ...
  layout(){
    ...
    this.GUT = w < 560 ? 64 : 96;      /* 右缘标签槽：锚/MA 标签专用，K线不进入 */
  ...
  _drawBands(ctx){
    ...
    const A = {
      low:  hot === 'low' ? 0.10 : (hot ? 0.015 : 0.06),
      mid:  hot === 'mid' ? 0.10 : (hot ? 0.015 : 0.045),
      high: hot === 'high' ? 0.10 : (hot ? 0.015 : 0.05),
    };
    ctx.save();
    ctx.beginPath();
    ...
    /* 色带左缘 3px 竖色条：区间识别不依赖图内大字 */
    ctx.fillStyle = hexA(this.C.red, .45);   ctx.fillRect(this.px0, this.TL, 3, Math.max(0, yHigh - this.TL));
    ctx.fillStyle = hexA(this.C.blue, .45);  ctx.fillRect(this.px0, yHigh, 3, Math.max(0, yLow - yHigh));
    ctx.fillStyle = hexA(this.C.green, .45); ctx.fillRect(this.px0, yLow, 3, Math.max(0, this.TL + this.priceH - yLow));
    ctx.setLineDash([6, 4]);
    ctx.lineWidth = 1;
  ...
  overlay(){
    ...
    inViewRows.forEach(rr => {
      ...
      const priceTxt = fmtSR(vv);
      const methodTxt = '';
      const badgeW = this._isMobile ? (40 + priceTxt.length * 6.6) : (46 + priceTxt.length * 6.6);
      ...
      if(inViewRows.length > 5 && ev === 'C') return;
    ...
    packLbl(srLbls, 24).forEach(it => {
      ...
      s.push('<text x="40" y="' + (it.yy + 4).toFixed(1) + '" font-size="11" font-weight="600" fill="' + it.col + '" font-family="SF Mono,monospace">' + csym(this.st) + it.price + '</text>');
      if(it.methodTxt){
    ...
    if(pr.hasV){
      ...
      if(zoneCard && zoneY + zoneCard.h <= priceB - 4){
        s.push('<g style="filter:drop-shadow(0 4px 12px rgba(0,0,0,.16))">' + zoneCard.svg(zoneY) + '</g>');
      }
      ...
      zLabels.forEach(z => {
        if(true) return; /* 区内大字标签已移除：区间由色带左缘色条+左上现价卡表达 */
        const yy = z.y0 + 15;
    ...
    } else if(isFinite(+st.v_low) && isFinite(+st.v_high) && +st.v_low > 0 && +st.v_high > +st.v_low){
      ...
      if(hi - lo > 0){
        [0.382, 0.5, 0.618].forEach(rt => {
          ...
          if(isFinite(yy) && yy >= this.TL && yy <= priceB){
            ...
            /* 斐波那契标签默认不上图（数值见数据墙 S/R 明细与十字光标提示） */
          }
        });
      }
    }
    ...
    [5,10,20,60,120,250].forEach(w => {
      ...
      if(W >= 560) rightItems.push({y: yy, lab: 'M' + w + ' ' + fmt2(v), col: this._maCol[w], bg: hexA(this.C.bg2, .92)});
    });

    const chW = small ? 5.2 : 6.3;
    const maxChars = Math.max(4, Math.floor((this.GUT - 14) / chW));
    packLblMutual(srLbls, rightItems, 24, 19, 20).forEach(it => {
      ...
      s.push('<rect x="' + (xR - wdt).toFixed(0) + '" y="' + (it.yy - 8).toFixed(1) + '" width="' + wdt.toFixed(0) + '" height="16" rx="4" fill="' + it.bg + '"/>'
           + '<text x="' + (xR - wdt + 5).toFixed(0) + '" y="' + (it.yy + 3.5).toFixed(1) + '" font-size="' + (small ? 10 : 10.5) + '" font-weight="600" fill="' + it.col + '" font-family="SF Mono,monospace">' + lab + '</text>');
    });
  ...
  toggleBand(){
    ...
    this._redrawCanvas();
    this.overlay();   /* 同步 SVG 覆盖层：锚线/现价卡随色带显隐（原实现只重绘 Canvas） */
    this._redrawCanvas();
  }
  ...
  _updateSrFoot(st){
    ...
    inView.forEach((x, i) => { x._n = inView.slice(0, i + 1).filter(y => y.s === x.s).length; });
    const capHTML = inView.map((x, i) =>
      '<button class="cap ' + (x.s ? 's' : 'r') + '" title="' + (x.method || '') + ' ' + csym(this.st) + fmt2(+x.level) + '" onclick="KENGINE.scrollToPrice(' + (+x.level) + ')">'
      + '<i></i><span class="v">' + (x.s ? 'S' : 'R') + x._n + ' ' + csym(this.st) + fmt2(+x.level) + '</span></button>');
    foot.innerHTML = capHTML.length > 6 ? capHTML.slice(0, 6).join('') + '<span class="cap" style="cursor:default"><span class="v">共 ' + capHTML.length + ' 个</span></span>' : capHTML.join('');
  }
